...
std::vector<Engine::Graphics::Command const *> Editor::EditorGUIRenderingStrategy::GetRenderingCommands(
    ...
    Engine::Graphics::Image<2> &renderTarget, std::optional<Engine::Graphics::Image<2>> &depthTarget) {

  auto renderingCommands =
      subStrategy->GetRenderingCommands(request, uniformBinder, descriptorAllocator, descriptorWriter, renderBuffer, depthTarget);
...
void Editor::EditorGUIRenderingStrategy::CreateRenderBuffer() {
  renderBuffer = objectManager->AllocateImage(VK_FORMAT_R8G8B8A8_UNORM, targetResolution,
